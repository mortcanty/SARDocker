...
def getmat(fn,m,x0,y0,cols,rows,bands):
...
#      T11 (k1)
        ...
        k1 = m*b.ReadAsArray(x0,y0,cols,rows)
#      T12  (a1)
...
#      T22 (xsi1)
        ...
        xsi1 = m*b.ReadAsArray(x0,y0,cols,rows)    
#      T23 (b1)        
...
#      T33 (zeta1)
        ...
        zeta1 = m*b.ReadAsArray(x0,y0,cols,rows) 
...
--------------------------------------------'''%sys.argv[0]
    ...
    fns = args[0].split(',')
    outfn = args[1]
    m = float(eval(args[2])) # equivalent number of looks
    n = float(len(fns))      # number of images
    print '==============================================='
...
#  output file
    ...
    start = time.time()
    sumlogdet = 0
    k = 0; a = 0; rho = 0; xsi = 0; b = 0; zeta = 0
    for fn in fns:
        result = getmat(fn,m,x0,y0,cols,rows,bands)
        if p==3:
