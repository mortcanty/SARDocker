...
def logstr(x):
# logarithmic stretch for SAR data
...
def logar(band,dt):
    if dt==1:
        ...
        band = np.nan_to_num(np.fromstring(band,dtype=np.float32))
    band = logstr(band)
...
def stretch(redband,greenband,blueband,enhance,dt):
    if enhance=='linear2pc':
        ...
        return (logar(redband,dt),logar(greenband,dt),logar(blueband,dt))
    else: # do nothing
